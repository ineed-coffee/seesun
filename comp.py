...
def img_capture():
    ...
    return redirect(url_for('show_capture',name=stamp))

def stamp_file(name):
    return 'temp/' + name +".jpg"

# 이미지
@app.route('/img/image/<name>', methods=['POST', 'GET'])
def show_capture(name):
...
def error():
    speech = get_speech()
    speech.play_audio('unknown_command',guide=True,wait=True)
